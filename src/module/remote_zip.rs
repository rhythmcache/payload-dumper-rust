        ...
        for i in 0..tail_buffer.len().saturating_sub(search_pattern.len()) {
            if tail_buffer[i..].starts_with(search_pattern) {
                ...
                if let Some(colon_pos) = tail_buffer[i..].iter().position(|&b| b == b':') {
                    ...
                    if let Some(end) = tail_buffer[start..].iter().position(|&b| b == b',' || b == b'\n' || b == b'\r') {
                        ...
                        if parts.len() >= 2 {
                            if let (Ok(offset), Ok(size)) = (parts[0].parse::<u64>(), parts[1].parse::<u64>()) {
    ...
    fn find_payload_via_zip_structure(mut http_reader: HttpReader) -> Result<Self> {
        ...
        for _entry_num in 0..real_num_entries {
            ...
            if compression_method != 0 {
                ...
                http_reader.seek(SeekFrom::Current((filename_len + extra_len + comment_len) as i64))?;
                continue;
            }
                        ...
                        while field_pos + 8 <= pos + 4 + data_size {
                            if local_header_offset == 0xFFFFFFFF {
                                local_header_offset = u64::from_le_bytes([
                                    extra_data[field_pos], extra_data[field_pos + 1],
                                    extra_data[field_pos + 2], extra_data[field_pos + 3],
                                    extra_data[field_pos + 4], extra_data[field_pos + 5],
                                    extra_data[field_pos + 6], extra_data[field_pos + 7],
                                ]);
                                field_pos += 8;
            ...
            if filename == b"payload.bin" || filename.ends_with(b"/payload.bin") {
                ...
                let payload_offset = local_header_offset + 30 + local_filename_len + local_extra_len;
...
impl Read for RemoteZipReader {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        let bytes_read = self.http_reader
            .read_at(self.payload_offset + self.current_position, &mut buf[..to_read])?;
