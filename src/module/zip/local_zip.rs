...
/*
    ...
    pub fn load_payload_entry(&mut self) -> IoResult<()> {
        let payload_names = ["payload.bin", "payload", "update.bin"];

        for name in &payload_names {
            if let Some(entry) = self.decoder.get_entry(name) {
                let mut entry = entry.clone();
        ...
        let best_entry = {
            let mut best_entry: Option<ZipEntry> = None;
            for entry in self.decoder.entries() {
                if entry.name.ends_with(".bin") || best_entry.is_none() {
                    if let Some(ref current) = best_entry {
                        if entry.uncompressed_size > current.uncompressed_size {
